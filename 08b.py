...
def decode_numbers(input):
    ...
    for line in input:
        ...
        signals = [to_binary(s.strip()) for s in signals.strip().split(' ')]
        output = [to_binary(o.strip()) for o in output.strip().split(' ')]

        for perm in permutations([0, 1, 2, 3, 4, 5, 6]):
            for signal in signals:
                transformed = apply_permutation(signal, perm)
                if transformed not in DIGITS.keys():
                    break
            else:
                output_signal = int(''.join([DIGITS[apply_permutation(out, perm)] for out in output]))
                print(f'Found permutation: {perm}, Output Signal: {output_signal}')
                total += output_signal
                break
        else:
            print('Did not find a permutation.')
