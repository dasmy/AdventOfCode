...
for rucksacks in zip(*[iter(input)]*3):
    rucksacks = tuple(set(i for i in r) for r in rucksacks)

    in_all = set.intersection(*rucksacks)
    assert len(in_all) == 1
    in_all = list(in_all)[0]

    if in_all.islower():
        prio = ord(in_all) - ord('a') + 1
    else:
        prio = ord(in_all) - ord('A') + 27
