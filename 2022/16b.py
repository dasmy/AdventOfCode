...
def traverse(time, pos, opened):
    if time == available_time:
        ...
        if valves[pos].rate > 0 and pos not in opened:
            ...
            released, released_open = explore_tunnels(time + 1, pos, opened + (pos, ))
            released += remaining_time * valves[pos].rate
            if released > best[0]:
                best = (released, released_open)

        return best


released_1, open = traverse(4, 'AA', tuple())
released_2, open = traverse(4, 'AA', open)
print(released_1 + released_2)
